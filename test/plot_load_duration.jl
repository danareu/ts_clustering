...
for (j, w) in enumerate(keys(warping_window))
    ...
    D = TSClustering.define_distance(w=warping_window[w], data_clustering=data_clustering)
    ...
    for t ∈ technology
        data_array[w,t,:] = sort(TSClustering.upsample_time_series(weight=weights, cluster_dict=sc, technology=t, region=region, config=config, tot_sum=false), rev=true)
    end
end
